...
- 管理员在“积分 → 积分流水”统一查看已入账流水与 `active`、`pending`、`dead_letter` 预留，通过状态筛选查看“异常扣费”或“需要人工处理”。异常扣费包括已经失败的 `pending` 重试和 `dead_letter`，不包括正常调用中或尚未尝试结算的记录。
- 未结算记录明确标为“预留，未扣费”，不显示入账后余额；已结算记录使用真实的余额快照。合并仅发生在管理查询中，积分预留和余额变动流水继续使用各自的数据模型。
- 在记录行点击“人工处理”，核对用户、接口、预留金额、请求 ID 与错误原因后选择处理方式。
- `active` 代表上游仍可能执行，只允许释放；不得人工确认扣费。
- `pending` 可确认扣费或释放。
- `dead_letter` 可重试、确认扣费或释放。
- 人工扣费在流水中记录管理员身份；完成结算后原预留由实际扣费流水替代，重复提交不会重复扣费。释放额度不产生余额变动，处理记录保留在操作审计中。
